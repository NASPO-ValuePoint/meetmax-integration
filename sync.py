...
for x in res:
    ...
    if x["attendee_role_id"] == 'NASPO2_SUPPLIER_ATT':
        ...
        if "status_code" in mm_co_record:
            cur.execute("UPDATE conferences_events.exchange_registration SET meetmax_id = %s WHERE id = %s",
                        (mm_co_record["id"], x["id"]))
            print(x["registration_id"] + " - Attendee Added.")
        else:
            cur.execute("UPDATE conferences_events.exchange_registration SET meetmax_status = %s WHERE id = %s",
                        (str(mm_co_record), x["id"]))
